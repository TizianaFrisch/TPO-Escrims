...
public class ScrimController {
    ...
    public ResponseEntity<Void> cancelar(@PathVariable Long id, @AuthenticationPrincipal com.uade.TrabajoPracticoProcesoDesarrollo.domain.entities.Usuario actor){
        var s = scrimService.obtener(id);
        // Política: si el creador está definido, sólo él puede cancelar; si no hay creador (datos legacy/tests), permitir para no bloquear flujo
    ...
    public ResponseEntity<Void> finalizar(@PathVariable Long id, @AuthenticationPrincipal com.uade.TrabajoPracticoProcesoDesarrollo.domain.entities.Usuario actor){
        var s = scrimService.obtener(id);
        // Política: si el creador está definido, sólo él puede finalizar; si no hay creador (datos legacy/tests), permitir para no bloquear flujo
        if (s.getCreador() != null) {
            if (actor == null || !java.util.Objects.equals(s.getCreador().getId(), actor.getId())) {
                return ResponseEntity.status(403).build();
            }
