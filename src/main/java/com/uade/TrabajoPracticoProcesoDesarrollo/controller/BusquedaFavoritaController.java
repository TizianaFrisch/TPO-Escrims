...
public class BusquedaFavoritaController {
    ...
    @PreAuthorize("@principalResolver.isCurrentUser(#dto.usuarioId, authentication) or hasRole('ADMINISTRADOR')")
    @PostMapping
    ...
    @PreAuthorize("@principalResolver.isCurrentUser(#userId, authentication) or hasRole('ADMINISTRADOR')")
    @GetMapping
    public List<BusquedaFavorita> mine(@RequestParam Long userId){
        return repo.findByUsuarioId(userId);
    }

    @PreAuthorize("hasRole('ADMINISTRADOR') or @busquedaFavoritaSecurity.isOwner(#id, authentication)")
    @DeleteMapping("{id}")
    public void delete(@PathVariable Long id){
        // Additional runtime check is performed inside the security bean; repository deletion proceeds if allowed.
        repo.deleteById(id);
    }
}
